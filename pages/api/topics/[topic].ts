...
export default async function handler(
    ...
    const { p, day } = req.query;

    const reqLink = req.query.topic;
    let reqPage = p == null ? 1 : p;
    const today = day == null ? new Date().toISOString().split('T')[0] : day;

    const pageCount = await checkPageNumber(reqLink as string);
    if (parseInt(reqPage.toString()) > pageCount) {
        reqPage = pageCount.toString();
    }
    if (parseInt(reqPage.toString()) < 1) {
        reqPage = "1";
    }
    ...
    const configEntries: AxiosRequestConfig = {
        method: 'get',
        url: `https://eksisozluk.com/${reqLink}?day=${today}&p=${reqPage}`,
        headers: {
